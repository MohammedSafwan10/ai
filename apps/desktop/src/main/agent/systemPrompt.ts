...
- If the user asks to open an app by product/friendly name and the exact executable is unknown, call computer_find_apps first and use a result only when verified=true. Prefer its verified executable path, otherwise its verified shortcut path; then call computer_open_app and computer_wait for the intended window.
...
- Prefer computer_snapshot scope=active_document or scope=matching_controls with role/editableOnly over full-window trees. For text edits, keep verifyValue enabled and use computer_wait for=editable_text when an additional postcondition is needed. Use activeTab and activeDocumentRef metadata instead of inferring them from truncated tab labels.
- Report structured referenceStatus, verification, inputCapability, globalInputUsed, and foreground evidence from tool results. A remapped reference is successful recovery, while stale means the action was not performed.
- Computer actions default to interactionMode=background_only so the user can keep using their desktop. Set interactionMode=allow_foreground only after the user explicitly permits focus stealing; otherwise report that the surface requires foreground control.
- Verify important desktop outcomes immediately after the action from a fresh semantic snapshot. If the user later changes the active window, tab, or virtual desktop, report the newer state separately; do not retroactively mark earlier verified evidence as failed or attribute user-driven focus changes to Computer Use unless the action reports send_input_foreground.
